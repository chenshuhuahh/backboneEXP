...
(function(factory) {
  ...
  // set a `X-Http-Method-Override` topNav.
  /*
  ...
  var internalOn = function(obj, name, callback, context, listening) {
    //  中间函数eventsApi,判断是哪种方式进行事件绑定，然后再进行递归或者循环或者直接处理
    //  将订阅者缓存到对象中
  ...
  // 对应上文的listening，解绑对另外一个对象的事件监听
  // 这里面的this上下文是监听者,而不是被监听者
  Events.stopListening = function(obj, name, callback) {
    var listeningTo = this._listeningTo;
    if (!listeningTo) return this;

    // 如果没有指定obj,就解绑所有的对别的对象的事件监听，如果指定了obj,就解绑对应obj
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      // 这里进行检查,如果没有监听，说明用这个函数是没有的，就直接break
      if (!listening) break;
  ...
  // 并且可以传递参数，直接按顺序写在第二、第三...个参数的位置即可
  // 依次传递给eventsApi、triggerApi、triggerEvents这几个函数并最终被使用
  ...
  var triggerApi = function(objEvents, name, callback, args) {
    if (objEvents) {
      ...
      // 处理对all事件进行监听的情况，假设A对象监听了B对象的all事件
      // 那么所有的B对象的事件都会被触发,并且会把事件名作为第一个函数参数
  ...
  _.extend(Model.prototype, Events, {
    ...
    set: function(key, val, options) {
      ...
      // changed用来存历史版本,因此backbone支持一个变量历史版本
      var changed = this.changed;
      // _previousAttributes存放着是历史版本变量
      // 也就是这次set之前这个model中有哪些键值对
      ...
      // Update the `id`.
      // 如果在set的时候传入了新的id,那么这个时候就可以更改id
  ...
  Backbone.sync = function(method, model, options) {
    ...
    // And an `X-HTTP-Method-Override` topNav.
